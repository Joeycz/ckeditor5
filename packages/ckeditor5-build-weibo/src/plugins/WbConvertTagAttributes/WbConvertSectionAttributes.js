export default function WbConvertSectionAttributes(editor) {
	...
	editor.conversion.for('downcast').add(dispatcher => {
		dispatcher.on('attribute', (evt, data, conversionApi) => {
				...
				if (data.attributeNewValue && data.attributeKey === 'style') {
					viewWriter.setAttribute(data.attributeKey, data.attributeNewValue, viewSection);
				} else {
					viewWriter.removeAttribute(data.attributeKey, viewSection);
				}
		} );
	} );
}